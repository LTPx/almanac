...
export async function GET(request: NextRequest) {
  ...
  const type = searchParams.get("type") as "units" | "lessons" | null;
  const onlyMissing = searchParams.get("onlyMissing") !== "false";

  if (!type || !["units", "lessons"].includes(type)) {
    return new Response("type must be 'units' or 'lessons'", { status: 400 });
  }
            ...
            batch.map(async (item) => {
              try {
                ...
                if (type === "units") {
                  ...
                  const enTranslation = await prisma.lessonTranslation.findUnique({
                ...
                send({
                  ...
                  total,
                  errors,
                  current: { id: item.id, name: sourceName, translated: translated.name }
                });
              } catch (err: any) {
                errors++;
                processed++;
